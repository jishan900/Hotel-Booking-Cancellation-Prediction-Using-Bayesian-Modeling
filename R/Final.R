...
set.seed(123)
train_indices <- sample(1:nrow(booking_data_prep), 0.01 * nrow(booking_data_prep))
...
priors <- get_prior(booking.status ~ lead.time + number.of.adults + number.of.children +
                      number.of.weekend.nights + number.of.week.nights + room.type + special.requests,
                    data = train_data,
...
model <- brm(booking.status ~ lead.time + number.of.adults + number.of.children +
               number.of.weekend.nights + number.of.week.nights + room.type + special.requests,
             data = train_data,
             prior = priors,
             #family = cumulative("logit"),
